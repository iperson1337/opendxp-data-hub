...
opendxp.plugin.datahub.configuration.graphql.configItem = Class.create(opendxp.element.abstract, {
    ...
    createSchemaStoreAndGrid: function (type) {
        ...
        if (type == "query") {
            columns.push({
                text: t('plugin_opendxp_datahub_configpanel_identifier_fields'),
                dataIndex: 'identifierFields',
                sortable: false,
                filter: 'string',
                flex: 1,
                renderer: function (value) {
                    if (Ext.isArray(value)) {
                        return Ext.util.Format.htmlEncode(value.join(', '));
                    }
                    return '';
                }
            });
            ...
            columns.push({
                xtype: 'actioncolumn',
                text: t('plugin_opendxp_datahub_configpanel_identifier_fields'),
                menuText: t('plugin_opendxp_datahub_configpanel_identifier_fields'),
                width: 60,
                items: [{
                    tooltip: t('plugin_opendxp_datahub_configpanel_identifier_fields'),
                    iconCls: "opendxp_icon_edit",
                    handler: function (grid, rowIndex) {
                        var record = grid.getStore().getAt(rowIndex);
                        this.openIdentifierFieldsDialog(record);
                    }.bind(this)
                }]
            });
        }
    ...
    /**
     * Edit the per-entity "identifierFields" list — fields that can be used to fetch a single
     * object via get<Class>(<field>: ...), e.g. getProduct(barcode: "..."). Suggestions are taken
     * from the entity's already-configured columns plus the always-available id/fullpath/uuid;
     * free entry is allowed so any class field name can be added.
     */
    openIdentifierFieldsDialog: function (record) {
        var current = record.get('identifierFields');
        if (!Ext.isArray(current)) {
            current = [];
        }

        // Only plain, single-column scalar fields make sense as an identifier and are safe for a
        // "WHERE <field> = <value>" lookup. Relations, composite types (quantityValue/numericRange),
        // field collections, assets, calculated and system values would break the query or be
        // meaningless, so they are excluded from the autocomplete list.
        var allowedDataTypes = [
            'input', 'numeric', 'select', 'country', 'language',
            'gender', 'slider', 'date', 'datetime', 'time',
            'firstname', 'lastname', 'email'
        ];

        var suggestions = ['id', 'fullpath', 'uuid'];
        var columnConfig = record.get('columnConfig');
        if (columnConfig && Ext.isArray(columnConfig.columns)) {
            Ext.each(columnConfig.columns, function (col) {
                if (col && col.attributes && col.attributes.attribute
                    && col.attributes.attribute.indexOf('~') === -1
                    && Ext.Array.contains(allowedDataTypes, col.attributes.dataType)) {
                    // skip brick/operator pseudo-attributes ("Brick~field") and non-scalar field types
                    suggestions.push(col.attributes.attribute);
                }
            });
        }

        var options = Ext.Array.unique(suggestions.concat(current));
        var store = Ext.create('Ext.data.Store', {
            fields: ['field'],
            data: Ext.Array.map(options, function (v) {
                return {field: v};
            })
        });
        ...
        var win = Ext.create('Ext.window.Window', {
            title: t('plugin_opendxp_datahub_configpanel_identifier_fields') + ' - ' + record.get('id'),
            width: 540,
            modal: true,
            layout: 'fit',
            items: [{
                ...
                items: [
                    {
                        xtype: 'displayfield',
                        value: t('plugin_opendxp_datahub_configpanel_identifier_fields_hint')
                    },
                    tagField
                ]
            }],
            buttons: [
                ...
                },
                {
                    ...
                    iconCls: 'opendxp_icon_apply',
                    handler: function () {
                        record.set('identifierFields', tagField.getValue());
                        win.close();
                    }
                }
            ]
